...
트리 아이템의 실행 로직을 처리하는 UseCase를 제공합니다.
"""
...
from core.usecases.interfaces.executor_interface import IExecutor


class ExecuteItemUseCase(IExecutor):
    """아이템 실행 UseCase 클래스
    
    트리 아이템의 실행 로직을 처리합니다.
    """
    
    def __init__(self, tree_view_model: Optional[ITreeViewModel] = None) -> None:
        """ExecuteItemUseCase 생성자
