...
def state_manager(mock_tree):
    ...
    # MTTreeStateManager 생성 시 __init__에서 mock_tree.to_dict()를 호출하여 _new_stage를 설정
...
def test_initial_state(state_manager, mock_tree):
    ...
    assert state_manager._new_stage == INITIAL_STATE # _new_stage 직접 확인
...
def test_set_initial_state_resets_history(state_manager, mock_tree):
    """set_initial_state가 히스토리를 초기화하고 _new_stage를 변경하는지 테스트"""
    # 초기 상태: _new_stage = INITIAL_STATE
    state_manager.new_undo(DUMMY_STATE_1) # _new_stage=S1, undo=[I]
    state_manager.undo(DUMMY_STATE_1)     # _new_stage=I, redo=[S1]

    assert not state_manager.can_undo()
    assert state_manager.can_redo()
    assert state_manager._new_stage == INITIAL_STATE
    ...
    assert state_manager._new_stage == new_initial_state_dict
    assert state_manager._undo_stack == []
    assert state_manager._redo_stack == []
    assert not state_manager.can_undo()
    assert not state_manager.can_redo()
...
def test_new_undo_basic(state_manager):
    """new_undo 기본 동작 테스트"""
    # 초기 상태: _new_stage = INITIAL_STATE
    current_stage_before_op = state_manager._new_stage # INITIAL_STATE
    ...
    assert state_manager._new_stage == DUMMY_STATE_1
    assert state_manager._redo_stack == []

def test_new_undo_clears_redo_stack(state_manager):
    """new_undo가 redo 스택을 비우는지 테스트"""
    # 초기 _new_stage = INITIAL_STATE
    state_manager.new_undo(DUMMY_STATE_1) # _new_stage=S1, undo=[I]
    state_manager.new_undo(DUMMY_STATE_2) # _new_stage=S2, undo=[I,S1]
    state_manager.undo(DUMMY_STATE_2)     # _new_stage=S1, undo=[I], redo=[S2]
    
    assert state_manager.can_redo()
    assert state_manager._redo_stack == [DUMMY_STATE_2]
    current_stage_before_new_op = state_manager._new_stage # DUMMY_STATE_1
    ...
    assert state_manager._new_stage == DUMMY_STATE_3
    assert not state_manager.can_redo()
    assert state_manager._redo_stack == []
...
def test_undo_basic(state_manager):
    ...
    current_stage_for_undo_arg = state_manager._new_stage # DUMMY_STATE_2
...
def test_undo_empty_stack(state_manager):
    """빈 undo 스택에서 undo 시도"""
    # 초기 _new_stage = INITIAL_STATE, undo_stack = []
    assert not state_manager.can_undo()
    assert state_manager.undo(state_manager._new_stage) is None
    assert state_manager._new_stage == INITIAL_STATE
    assert not state_manager.can_redo()
...
def test_redo_empty_stack(state_manager):
    """빈 redo 스택에서 redo 시도"""
    # 초기 _new_stage = INITIAL_STATE, redo_stack = []
    assert not state_manager.can_redo()
    assert state_manager.redo(state_manager._new_stage) is None
    assert state_manager._new_stage == INITIAL_STATE

def test_stack_limit_undo(state_manager): # max_history=3
    """undo 스택 최대치 제한 테스트"""
    # _new_stage = INITIAL_STATE
    state_manager.new_undo(DUMMY_STATE_1) # undo=[I], _new_stage=S1
    state_manager.new_undo(DUMMY_STATE_2) # undo=[I,S1], _new_stage=S2
    state_manager.new_undo(DUMMY_STATE_3) # undo=[I,S1,S2], _new_stage=S3
    assert len(state_manager._undo_stack) == 3
    
    state_4 = {"id": "state4"}
    state_manager.new_undo(state_4) # undo=[S1,S2,S3], _new_stage=S4 (INITIAL_STATE 제거됨)
    assert len(state_manager._undo_stack) == 3
    assert state_manager._undo_stack[0] == DUMMY_STATE_1 
    assert state_manager._undo_stack[-1] == DUMMY_STATE_3
    assert state_manager._new_stage == state_4
...
def test_stack_limit_redo(state_manager): # max_history=3
    ...
    state_manager.new_undo(s1)  # U:[s0], _new_stage=s1
    state_manager.new_undo(s2)  # U:[s0,s1], _new_stage=s2
    state_manager.new_undo(s3)  # U:[s0,s1,s2], _new_stage=s3
    state_manager.new_undo(s4)  # U:[s1,s2,s3], _new_stage=s4 (s0 제거)

    current_stage = s4
    current_stage = state_manager.undo(current_stage) # R:[s4], _new_stage=s3, U:[s1,s2]
    current_stage = state_manager.undo(current_stage) # R:[s4,s3], _new_stage=s2, U:[s1]
    current_stage = state_manager.undo(current_stage) # R:[s4,s3,s2], _new_stage=s1, U:[]
    assert len(state_manager._redo_stack) == 3
    
    state_manager._undo_stack = [{"id": "temp_for_undo"}] # 임시로 undo 가능하게 만듦
    # 현재 state_manager._new_stage == s1
    
    current_stage = state_manager.undo(s1) # R:[s3,s2,s1], _new_stage=temp_for_undo, U:[] (s4 제거)
    assert len(state_manager._redo_stack) == 3
    assert state_manager._redo_stack[0] == DUMMY_STATE_3 # s3
    assert state_manager._redo_stack[-1] == DUMMY_STATE_1 # s1
    assert state_manager._new_stage == {"id": "temp_for_undo"}
...
def test_subscription(state_manager):
    ...
    # new_undo는 조작 후 데이터를 받음, _new_stage는 DUMMY_STATE_1로 변경됨
    # _notify_subscribers는 변경된 _new_stage (DUMMY_STATE_1)를 전달
    state_manager.new_undo(DUMMY_STATE_1)
    mock_callback.assert_called_with(DUMMY_STATE_1)
    
    mock_callback.reset_mock()
    # undo 호출, 현재 _new_stage는 DUMMY_STATE_1, undo 후 _new_stage는 INITIAL_STATE가 됨
    # _notify_subscribers는 변경된 _new_stage (INITIAL_STATE)를 전달
    state_manager.undo(DUMMY_STATE_1)
    mock_callback.assert_called_with(INITIAL_STATE)

    mock_callback.reset_mock()
    # redo 호출, 현재 _new_stage는 INITIAL_STATE, redo 후 _new_stage는 DUMMY_STATE_1이 됨
    # _notify_subscribers는 변경된 _new_stage (DUMMY_STATE_1)를 전달
    ...
    state_manager.new_undo(DUMMY_STATE_2) # _new_stage는 DUMMY_STATE_2로 변경
...
def test_can_undo_redo_behavior(state_manager):
    ...
    current_stage_before_undo = state_manager._new_stage # DUMMY_STATE_1
    state_manager.undo(current_stage_before_undo) # _new_stage=I, undo=[], redo=[S1]
    assert not state_manager.can_undo()
    assert state_manager.can_redo()

    current_stage_before_redo = state_manager._new_stage # INITIAL_STATE
    state_manager.redo(current_stage_before_redo) # _new_stage=S1, undo=[I], redo=[]
    assert state_manager.can_undo()
    assert not state_manager.can_redo()
    assert state_manager._new_stage == DUMMY_STATE_1
...
def test_max_history_default(mock_tree):
    ...
    manager = MTTreeStateManager(tree=mock_tree) # _new_stage = INITIAL_STATE
    assert manager._max_history == 100

    current_stage = manager._new_stage # INITIAL_STATE
    for i in range(101):
        new_stage_data = {"id": f"state_{i}"}
        manager.new_undo(new_stage_data) # 이전 _new_stage가 undo 스택으로, new_stage_data가 현재 _new_stage가 됨
        # current_stage 변수는 이 루프 내에서는 직접 사용되지 않음 (manager 내부의 _new_stage가 중요)
        
    assert len(manager._undo_stack) == 100
    # 첫 new_undo: undo=[INITIAL_STATE], _new_stage=state_0
    # 두번째 new_undo: undo=[INITIAL_STATE, state_0], _new_stage=state_1
    # ...
    # 101번째 new_undo (i=100): undo=[state_0, ..., state_99], _new_stage=state_100 (INITIAL_STATE 제거)
    assert manager._undo_stack[0] == {"id": "state_0"}
    assert manager._undo_stack[-1] == {"id": "state_99"}
    assert manager._new_stage == {"id": "state_100"} 